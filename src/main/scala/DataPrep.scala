...
object DataPrep {
  ...
  def main(args: Array[String]) {
                   ...
                   map(x => x.split(",")).
                     map(x => 
                       ...
                       x(3).toString.
                          ...
                          replace("32","NV").replace("33","NH").replace("34","NJ").replace("35","NM").
    ...
    val sqlContext= SQLContext.getOrCreate(pass_two.sparkContext)
    import sqlContext._
    ...
    df.repartition(1).write.mode("overwrite").format("csv").save("/user/jjl359/project/df_for_logistic_regression")
